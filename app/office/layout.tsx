  ...
  return (
    <div className="min-h-screen text-sm flex">
      ...
      <aside className="w-50 bg-white drop-shadow-gray-400  drop-shadow-xl/50">
                ...
                {item.children && isOpen && (
                  <div className="ml-6 mt-1 mb-1">
                    {item.children.map((child) => {
                      const isChildActive = pathname === `/office${child.path}`;
                      return (
                        <Link
                          key={child.name}
        ...
        <Link
          href={"/"}
          className="absolute text-center w-42 bottom-4 left-4 bg-red-700 text-white px-4 py-2 rounded-md hover:bg-red-600"
        >
          Back to home
        </Link>
      </aside>
